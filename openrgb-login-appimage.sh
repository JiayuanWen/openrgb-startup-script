...
#For AppImage, the script must know where the appimage file is located inorder to launch the app
openrgb_autostart_apploc=/bin/true
...
if [ -n "$app_loc" ]
then
    sed -i "s|^openrgb_autostart_apploc=.*|openrgb_autostart_apploc=$app_loc|" $CONFIG_FILE

    echo "Script will use indicated AppImage to launch app. Launch the script again without any flags to execute."
    exit 0;
fi
